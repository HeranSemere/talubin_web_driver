...
class _CompletedTasksScreenState extends State<CompletedTasksScreen> {
            ...
            if(snapshot.connectionState==ConnectionState.done){
              ...
              if(snapshot.data as bool){
                return FutureBuilder<List<CompletedTask>>( //add a refresh functionality here
                  ...
                  CompletedTask().localise(snapshot.data!).then((value) {

                        loadCompletedTasks("no error");
               ...
               /* return CompletedTasksDatabase().createDB().then((value) {
                 ...
                 /* return FutureBuilder<List<CompletedTask>>(
                  ...
                  future: CompletedTaskLocal().localise(snapshot.data!),
                ...
                return loadCompletedTasks("error"); //just load from local
              }
            }else{
              return CircularProgressIndicator();
            }
  ...
  Widget loadCompletedTasks(var a){
  ...
  Widget savedCompletedTasks(){  ////maybe the problem is the future builder, use normal .then clause like the one in performance screen, and also make suredatabae is not empty
    ...
    builder: (context, snapshot){
      ...
      if(snapshot.connectionState==ConnectionState.done){
        ...
        if(snapshot.hasError){
          ...
          builder: (context, localCompletedTasks){

            if(localCompletedTasks.connectionState==ConnectionState.done){

              if(localCompletedTasks.hasError){
                  return Center(child: Text("error fetching local data"),); //error screen
                }else{

                  if(localCompletedTasks.data!.length == 0){
                    return Center(child: Text("No completed tasks yet"),);
                  }

               return ListView.builder(
                itemCount: localCompletedTasks.data!.length,
                itemBuilder: (context, index) {
                  print(localCompletedTasks.data);
                  print(localCompletedTasks.data);
                  return Container(
                    //height: 130,
                    margin: const EdgeInsets.all(0),
                                ...
                                children: <Widget>[
                                  ...
                                  Row(
                                    children: [
                                      Text('Completed date: '),
                                      Text(
                                        localCompletedTasks.data![index].completedDate
                                            .toString(),
                                        maxLines: 2,
                                        overflow: TextOverflow.ellipsis,
                                      ),
                                  ...
                                  SizedBox(height: 8.0),
                                  Text(
                                    localCompletedTasks.data![index].institutionName.toString()
